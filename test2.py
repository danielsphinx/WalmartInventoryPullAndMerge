...
def get_orders():
    ...
    next_cursor = f'?limit={limit}&status=Acknowledged'  #  - Initial next_cursor with your desired limit
    ...
    while True:
        ...
        print(response_data)
        all_orders.extend(response_data.get("list", {}).get("elements", {}).get("order", []))  # Append each order to the list

        # Check if there's a nextCursor value to continue pagination
        meta_data = response_data.get("list", {}).get("meta", {})
        if 'nextCursor' in meta_data and meta_data['nextCursor']:
            next_cursor = meta_data['nextCursor']
        else:
